...
theme_colors = {"dark": theme_dark, "light": theme_light, "pastel": theme_pastel}[
    SETTINGS["theme"]
]
...
def run_keypress(keys, app=None):
    if app:
        subprocess.Popen(
            f"bash -lc 'xdotool search --class {app} windowactivate key \"{keys}\"'",
            shell=True,
        )
    else:
...
# -------------------
def run_mouse_move(x, y):
    subprocess.run(["xdotool", "mousemove", str(x), str(y)])


def run_mouse_click(button="1", x=None, y=None):
    if x is not None and y is not None:
        run_mouse_move(x, y)
    subprocess.run(["xdotool", "click", button])


def run_mouse_drag(x1, y1, x2, y2, duration=0.5):
    subprocess.run(["xdotool", "mousemove", str(x1), str(y1)])
    subprocess.run(["xdotool", "mousedown", "1"])
    subprocess.run(["xdotool", "mousemove", "--sync", "--polar", str(x2), str(y2)])
    time.sleep(duration)
    subprocess.run(["xdotool", "mouseup", "1"])


def run_wait_for_window(app_class):
    while True:
        result = subprocess.run(
            ["xdotool", "search", "--class", app_class], capture_output=True, text=True
        )
...
def run_screenshot(path):
    subprocess.run(["grim", path])  # Wayland screenshot
...
descriptions = {
    ...
    # New blocks
    "Move Cursor": "Move mouse to X,Y coordinates.",
    "Mouse Click": "Click mouse button optionally at X,Y.",
    "Mouse Drag": "Drag mouse from X1,Y1 to X2,Y2.",
    "Wait for Window": "Wait until a window with class appears.",
    "Screenshot": "Take a screenshot and save to path.",
    "Random Choice": "Pick one option randomly and execute its block.",
}
...
def refresh_blocks():
    ...
    for idx, block in enumerate(macro_blocks):
        frame = tk.Frame(
            right_frame, bg=theme_colors["bg"], bd=1, relief=tk.SOLID, padx=3, pady=3
        )
        ...
        tk.Label(
            header,
            text=f"{block['type']}: {block['value'][:30]}{'...' if len(block['value']) > 30 else ''}",
            bg=theme_colors["bg"],
            fg=theme_colors["fg"],
        ).pack(side=tk.LEFT)
        ...
        tk.Button(
            header,
            text="Expand" if collapsed.get() else "Collapse",
            command=toggle,
        ...
        if not collapsed.get():
            ...
            tk.Button(
                frame,
                text="Remove",
                command=lambda i=idx: remove_block_by_index(i),
                bg=theme_colors["button_bg"],
                fg=theme_colors["button_fg"],
                bd=1,
            ).pack(pady=2)
...
def add_block():
    ...
    if block_type == "Launch App":
        ...
        val = simpledialog.askstring(
            f"{block_type} Value", descriptions.get(block_type, "Enter value")
        )
    if val is None:
        return
    app = None
    ...
    sub_blocks = []
    if block_type in ["Loop", "If Clipboard Contains", "Random Choice"]:
        sub_blocks = []
    macro_blocks.append(
        {"type": block_type, "value": val, "app": app, "sub_blocks": sub_blocks}
    )
...
def execute_block(block):
    ...
    if t == "Key Press":
        ...
        run_type_text(v, a)
    elif t == "Clipboard Write":
        run_clipboard_write(v)
    elif t == "Clipboard Append":
        run_clipboard_append(v)
    elif t == "Clipboard Read":
        print(run_clipboard_read())
    elif t == "Notify":
        run_notify(v)
    elif t == "Launch App":
        run_launch_app(v)
        ...
        parts = v.split(",")  # format: button,x,y
        button = parts[0] if len(parts) > 0 else "1"
        x = int(parts[1]) if len(parts) > 1 else None
        y = int(parts[2]) if len(parts) > 2 else None
        run_mouse_click(button, x, y)
    elif t == "Mouse Drag":
        x1, y1, x2, y2 = map(int, v.split(","))
        run_mouse_drag(x1, y1, x2, y2)
    elif t == "Wait for Window":
        run_wait_for_window(v)
    elif t == "Screenshot":
        run_screenshot(v)
    elif t == "Random Choice":
        choice = random.choice(block["sub_blocks"])
        execute_block(choice)
...
def export_macro():
    ...
    with open(path, "w") as f:
        ...
        def write_block(block, indent=1):
            ...
            sb = block["sub_blocks"]
            if t == "Key Press":
                f.write(f"{prefix}subprocess.run(['wtype','{v}'])\n")
            elif t == "Type Text":
                for ch in v:
                    f.write(
                        f"{prefix}subprocess.run(['wtype','{ch}']); time.sleep({SETTINGS['typing_speed']})\n"
                    )
            elif t == "Clipboard Write":
                f.write(f"{prefix}subprocess.run(['wl-copy'],input='{v}'.encode())\n")
            elif t == "Clipboard Append":
                f.write(
                    f"{prefix}curr=subprocess.run(['wl-paste'],capture_output=True,text=True).stdout; subprocess.run(['wl-copy'],input=(curr+'{v}').encode())\n"
                )
            elif t == "Clipboard Read":
                f.write(
                    f"{prefix}import subprocess; print(subprocess.run(['wl-paste'],capture_output=True,text=True).stdout)\n"
                )
            elif t == "Notify":
                f.write(f"{prefix}subprocess.run(['notify-send','{v}'])\n")
            elif t == "Launch App":
                f.write(
                    f"{prefix}subprocess.Popen(f\"bash -lc '{v}'\",shell=True,cwd=os.path.expanduser('~'))\n"
                )
            elif t == "Kill App":
                f.write(f"{prefix}subprocess.run(['killall','{v}'])\n")
            elif t == "Wait":
                f.write(f"{prefix}time.sleep({v})\n")
            elif t == "Wait Random":
                p = v.split(",")
                ...
                if a:
                    ...
                    f.write(
                        f"{prefix}subprocess.Popen(['{SETTINGS['default_browser']}','{url}'])\n"
                    )
            elif t == "Open File":
                f.write(f"{prefix}subprocess.Popen(['xdg-open','{v}'])\n")
            elif t == "Volume Up":
                f.write(
                    f"{prefix}subprocess.run(['pactl','set-sink-volume','@DEFAULT_SINK@','+5%'])\n"
                )
            elif t == "Volume Down":
                f.write(
                    f"{prefix}subprocess.run(['pactl','set-sink-volume','@DEFAULT_SINK@','-5%'])\n"
                ...
                f.write(
                    f"{prefix}subprocess.run(['pactl','set-sink-mute','@DEFAULT_SINK@','toggle'])\n"
                )
            elif t == "Loop":
                f.write(f"{prefix}for _ in range({v}):\n")
                for sb in sb:
                    write_block(sb, indent + 1)
            elif t == "If Clipboard Contains":
                f.write(f"{prefix}import subprocess\n")
                f.write(
                    f"{prefix}if subprocess.run(['wl-paste'],capture_output=True,text=True).stdout.strip()=='{v}':\n"
                )
                for sb in sb:
                    write_block(sb, indent + 1)
            elif t == "Move Cursor":
                x, y = map(int, v.split(","))
                f.write(
                    f"{prefix}subprocess.run(['xdotool','mousemove','{x}','{y}'])\n"
                )
            elif t == "Mouse Click":
                parts = v.split(",")
                button = parts[0] if len(parts) > 0 else "1"
                ...
                f.write(f"{prefix}subprocess.run(['xdotool','click','{button}'])\n")
            elif t == "Mouse Drag":
                x1, y1, x2, y2 = map(int, v.split(","))
                f.write(
                    f"{prefix}subprocess.run(['xdotool','mousemove','{x1}','{y1}'])\n"
                )
                f.write(f"{prefix}subprocess.run(['xdotool','mousedown','1'])\n")
                f.write(
                    f"{prefix}subprocess.run(['xdotool','mousemove','{x2}','{y2}'])\n"
                )
                f.write(f"{prefix}subprocess.run(['xdotool','mouseup','1'])\n")
            elif t == "Wait for Window":
                f.write(f"{prefix}import subprocess, time\n")
                f.write(f"{prefix}while True:\n")
                f.write(
                    f"{prefix}    result = subprocess.run(['xdotool','search','--class','{v}'],capture_output=True,text=True)\n"
                )
                f.write(f"{prefix}    if result.stdout.strip(): break\n")
                f.write(f"{prefix}    time.sleep(0.2)\n")
            elif t == "Screenshot":
                f.write(f"{prefix}subprocess.run(['grim','{v}'])\n")
            elif t == "Random Choice":
                f.write(f"{prefix}import random\n")
                f.write(f"{prefix}execute_block(random.choice({sb}))\n")
...
# Left panel
left_frame = tk.Frame(
    root, bg=theme_colors["bg"], bd=1, relief=tk.SOLID, padx=5, pady=5
)
left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)
tk.Label(
    left_frame, text="Block Type:", bg=theme_colors["bg"], fg=theme_colors["fg"]
).pack(pady=2)
...
tk.Label(
    right_frame, text="Macro Blocks:", bg=theme_colors["bg"], fg=theme_colors["fg"]
).pack()
