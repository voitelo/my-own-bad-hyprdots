...
menu() {
    printf "%s\n" "$@" | rofi -config /home/leg/.config/rofi/application-launcher/config.rasi -style /home/leg/.config/rofi/application-launcher/theme.rasi -dmenu -p "Dashboard >"
}
...
install_menu() {
    ...
    # ask for package search term
    query=$(rofi -config /home/leg/.config/rofi/application-launcher/config.rasi -style /home/leg/.config/rofi/application-launcher/theme.rasi -dmenu -p "Search packages >")
    [ -z "$query" ] && main_menu

    # search depending on helper
    if [ "$helper" = "pacman" ]; then
        results=$(pacman -Ss "$query" | awk '{print $1" "$2" "$3}')
    else
        results=$($helper -Ss "$query" | awk '{print $1" "$2" "$3}')
    fi

    # if no results, back to menu
    [ -z "$results" ] && notify-send "No results for: $query" && install_menu

    # let user pick a package
    package=$(printf "%s\n" "$results" | rofi -config /home/leg/.config/rofi/application-launcher/config.rasi -style /home/leg/.config/rofi/application-launcher/theme.rasi -dmenu -p "Install which? >")
    [ -z "$package" ] && install_menu

    # extract package name (cut repo/pkgname)
    pkg=$(echo "$package" | awk -F/ '{print $2}' | awk '{print $1}')

    # confirm install
    confirm=$(printf "Yes\nNo" | rofi -config /home/leg/.config/rofi/application-launcher/config.rasi -style /home/leg/.config/rofi/application-launcher/theme.rasi -dmenu -p "Install $pkg ?")
    [ "$confirm" = "Yes" ] && kitty -e $helper -S "$pkg"
