...
# -------------------------------
printf "Is your root filesystem Btrfs? Y/y/N/n: "
read btrfs_question
case "$btrfs_question" in
  [Yy])
      if pacman -Qs btrfs-progs-git &>/dev/null; then
          log "Running safe Btrfs maintenance..."
          # Safe scrub
          sudo btrfs scrub start / 
          # Quick targeted balance to free lightly used chunks
          sudo btrfs balance start -dusage=50 /
          ok "Btrfs scrub and quick balance started."

          # -------------------------------
          # Btrfs maintenance status summary
          # -------------------------------
          log "Checking Btrfs maintenance status..."

          # Scrub status
          scrub_status=$(sudo btrfs scrub status / | grep "Status:" | awk '{print $2}')
          case "$scrub_status" in
              finished)
                  echo -e "\033[1;32m[OK] Scrub finished\033[0m"
                  ;;
              running)
                  echo -e "\033[1;34m[INFO] Scrub running\033[0m"
                  ;;
              *)
                  echo -e "\033[1;33m[WARN] Scrub status unknown\033[0m"
                  ;;
          esac

          # Balance status
          balance_status=$(sudo btrfs balance status / | grep "Status:" | awk -F': ' '{print $2}')
          case "$balance_status" in
              "no balance found"|"finished")
                  echo -e "\033[1;32m[OK] Balance finished / no balance needed\033[0m"
                  ;;
              running*)
                  echo -e "\033[1;34m[INFO] Balance running\033[0m"
                  ;;
              *)
                  echo -e "\033[1;33m[WARN] Balance status unknown\033[0m"
                  ;;
          esac
      else
      ...
      ;;
  [Nn])
      echo "Skipping Btrfs maintenance."
      ;;
  *)
      echo "Invalid input. Skipping Btrfs maintenance."
      ;;
esac
