...
# Show menu with rofi
choice=$(echo -e "$menu" | rofi -dmenu -config "$HOME/.config/rofi/application-launcher/config.rasi" -p "Run or manage a script:")
...
if [[ "$choice" == "Delete Script" ]]; then
  ...
  del_menu=$(printf "%s\n%s\nCancel" "test.sh" "$scripts")
  del_choice=$(echo -e "$del_menu" | rofi -dmenu -config "$HOME/.config/rofi/application-launcher/config.rasi" -p "Delete which script?")
