...
BASE = Path.home() / ".local/share/rofi_calc/"
...
def rofi(prompt, items):
    ...
    system(f"cat '{temp}' | rofi -dmenu -p '{prompt}' > '{tmp_out}'")
...
def confirm(prompt):
    c1 = rofi(prompt, ["no", "yes"])
    if c1 != "yes":
        return False
    c2 = rofi("are you VERY sure?", ["no", "yes"])
    return c2 == "yes"
...
def main():
    ...
    while True:
        ...
        for e in entries:
            ...
            answer_choice = rofi("calculator >", [result, "back"])
