...
# Show walker menu with rofi
choice=$(echo -e "$menu" | rofi -dmenu -config "$HOME/.config/rofi/application-launcher/config.rasi" -p "Clipboard history (or type new):")
...
"Delete entry")
  ...
  del=$(tac "$histfile" | rofi -dmenu -config "$HOME/.config/rofi/application-launcher/config.rasi" -p "Select entry to delete:")
