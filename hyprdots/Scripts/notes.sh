...
newnote() {
  ...
  name="$(echo "$default_name" | rofi -dmenu -p "Enter note name:")"
  [[ -z "$name" ]] && return
...
delete_note() {
  notes=$(ls -t1 "$folder")
  [[ -z "$notes" ]] && return
  file_to_delete=$(echo -e "$notes" | rofi -dmenu -p "Select note to delete:")
...
selected() {
  ...
  choice=$(echo -e "$menu" | rofi -dmenu -p "Select or type note:")
