...
newnote() {
  ...
  name="$(echo "$default_name" | walker --dmenu -c -sb "#a3be8c" -nf "#d8dee9" -p "Enter note name: " <&-)"
  [[ -z "$name" ]] && return
...
delete_note() {
  notes=$(ls -t1 "$folder")
  [[ -z "$notes" ]] && return
  file_to_delete=$(echo -e "$notes" | walker --dmenu -c -l 10 -i -p "Select note to delete: ")
...
selected() {
  ...
  choice=$(echo -e "$menu" | walker --dmenu -c -l 12 -i -p "Select or type note: ")
