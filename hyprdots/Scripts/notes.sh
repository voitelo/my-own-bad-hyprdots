...
newnote() {
  ...
  name="$(echo "$default_name" | rofi -dmenu -config "$HOME/.config/rofi/application-launcher/config.rasi" -p "Enter note name:")"
  [[ -z "$name" ]] && return
...
delete_note() {
  notes=$(ls -t1 "$folder")
  [[ -z "$notes" ]] && return
  file_to_delete=$(echo -e "$notes" | rofi -dmenu -config "$HOME/.config/rofi/application-launcher/config.rasi" -p "Select note to delete:")
...
selected() {
  ...
  choice=$(echo -e "$menu" | rofi -dmenu -config "$HOME/.config/rofi/application-launcher/config.rasi" -p "Select or type note:")
