...
choice=$(echo -e "$menu" | fuzzel --dmenu -p "Window Manager:")
...
if [[ "$choice" == "Minimize Window" ]]; then
  ...
  win_choice=$(echo -e "$win_list" | fuzzel --dmenu -p "Select window to minimize:")
...
if [[ "$choice" == "Un-minimize Window" ]]; then
  ...
  un_choice=$(echo -e "$unmin_menu" | fuzzel --dmenu -p "Select window to un-minimize:")
  [[ -z "$un_choice" ]] && exit
